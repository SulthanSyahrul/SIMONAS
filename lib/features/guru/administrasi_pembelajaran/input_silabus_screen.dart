...
/// Screen untuk input Silabus
/// Menyimpan data ke tabel Silabus dengan struktur:
/// - silabus_id
/// - user_id
/// - kelas_id
/// - mata_pelajaran_id
...
/// TODO: Metadata file disimpan di Firestore (tabel Silabus)
/// TODO: Validasi file hanya PDF dan ukuran maksimal
class InputSilabusScreen extends StatefulWidget {
  const InputSilabusScreen({super.key});

  @override
  State<InputSilabusScreen> createState() => _InputSilabusScreenState();
}

class _InputSilabusScreenState extends State<InputSilabusScreen> {
  // Form key
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Input Silabus'),
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        const Expanded(
                          child: Text(
                            'Upload silabus pembelajaran per semester',
                            style: TextStyle(
                              color: AppColors.info,
                              fontSize: 14,
                            ),
  ...
  void _handleFileUpload() {
    ...
    setState(() {
      _selectedFileName = 'Silabus_${_selectedMataPelajaran ?? 'Dokumen'}_${_selectedKelas ?? ''}_2025.pdf';
    });
  ...
  /// TODO: Simpan metadata ke Firestore (tabel Silabus)
  void _handleSimpan() {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('Simpan silabus dengan data:'),
              const SizedBox(height: 12),
  ...
  Future<void> _saveSilabus() async {
    ...
    messenger.showSnackBar(
      const SnackBar(
        content: Text('Silabus berhasil disimpan'),
        backgroundColor: AppColors.success,
        duration: Duration(seconds: 2),
      ),
    );
